...
def register_consulta_handlers(bot: TeleBot):
    ...
    def check_access(message):
        ...
        if user_id in ultimo_uso and (ahora - ultimo_uso[user_id]) < 2:
            bot.reply_to(message, "Espera 2 segundos entre consultas.")
            return None
    ...
    @bot.message_handler(func=lambda m: "Consultar Orden" in m.text)
    def ask_orden(message):
        if not check_access(message): return
        bot.send_message(message.chat.id, "Escribe el número de orden:")
        bot.set_state(message.from_user.id, EnelState.waiting_for_orden, message.chat.id)

    @bot.message_handler(func=lambda m: "Consultar Rótulo" in m.text)
    def ask_rotulo(message):
        if not check_access(message): return
        bot.send_message(message.chat.id, "Escribe el número de rótulo:")
        bot.set_state(message.from_user.id, EnelState.waiting_for_rotulo, message.chat.id)

    # ================== MANEJADORES DE ESTADO (INPUT) ==================
    @bot.message_handler(state=EnelState.waiting_for_orden)
    def process_orden(message):
        user = check_access(message)
        if not user: return
